...
function makeFour() {
	window.localStorage.setItem('married', 'true');
}
...
function makeSix() {
	//Ваш код
	let JSONobj6 = JSON.stringify(alice);
...
function makeSeven() {
	let JSONobj7 = JSON.stringify(greatBritain);
...
function makeEight() {
	...
	let JSONobj8 = JSON.stringify(fruits);
...
function makeNine() {
	window.localStorage.setItem('arrayNine', JSON.stringify(five));
}
...
//Подсказка: Для преобразования массива в строку используйте метод JSON.stringify()
const freshFruits = ["яблоко", "банан", "апельсин"]
function makeTen() {
	//Ваш код
	window.localStorage.setItem('arrayTen', JSON.stringify(freshFruits));
...
function makeEleven() {
	window.localStorage.setItem('arrayEleven', JSON.stringify(friends));
}
...
const button23 = document.querySelector('.b-23')
const showCart = () => {
	...
	cartItems.forEach((element) => {
		...
		practicum23.after(box);
	 })
};
button23.addEventListener('click', showCart);
